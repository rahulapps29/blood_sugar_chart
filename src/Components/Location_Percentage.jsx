...
const LocationPercentage = ({ data }) => {
  // State to store location counts
  const [locationCountsX, setLocationCountsX] = useState({});
  ...
  useEffect(() => {
    if (data.tasks && data.tasks.length > 0) {
      ...
      setLocationCountsX(counts); // Update state with calculated counts
    }
  }, [data]); // Dependency array includes 'data' to re-run when 'data' changes

  const locations_dict_temp = Object.entries(locationCountsX).map(
  ...
  const countList = listOfLists.map(
    (subList) => (subList[1] / totalCount) * 100
  );
